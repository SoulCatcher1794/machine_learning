...
# Create a train/validation split from the original training set
...
# Define learning rates/shrinkage to evaluate for Gradient Boosting
learning_rates = [1.0, 0.1, 0.03]

# Initialize dictionaries to store results and best parameters
gbr_results = {} # Will hold train and validation MSE for each learning rate

# Create a Gradient Boosting Regressor for each learning rate and compute staged predictions
for rate in learning_rates:
    ...
    # Compute staged predictions (300 iterations) for training and validation sets and calculate MSE
    ...
    # Store the results in the gbr_results dictionary with learning rate as the key
    gbr_results[rate] = {
        "train_mse": train_mse,
        "val_mse": val_mse
    }

best_params_gbr = {} # Will hold the best learning rate and iteration
best_val_mse_overall = None # Initialize to a large value for comparison

# Determine best parameters based on the minimum validation MSE across all learning rates
for rate, val_mse in gbr_results.items():
    # Find the iteration (1-based index) with the minimum validation MSE for the current learning rate
    best_iter = int(np.argmin(val_mse["val_mse"]) + 1)
    
    # Get the best validation MSE for the current learning rate
    best_val_mse = val_mse["val_mse"][best_iter - 1]

    # Update the overall best parameters if this learning rate yields a better validation MSE
    # Assign values if first iteration
    if (not best_params_gbr) or (best_val_mse < best_val_mse_overall):
        # Update the overall best validation MSE across all learning rates
        best_val_mse_overall = best_val_mse

        # If the current learning rate yields a better validation MSE, update the best parameters
        best_params_gbr = {
            "learning_rate": rate,
            "best_iter": best_iter
        }

# Once the best parameters are determined, fit a final Gradient Boosting Regressor on the full training set
...
# Train the final model with the optimal learning rate and number of estimators on the full training set
final_gbr.fit(X_train_reg, y_train_reg)
final_gbr_predictions = final_gbr.predict(X_test_reg)

# Calculate the MSE on the test set for the final Gradient Boosting Regressor
mse_gbr_test = mean_squared_error(y_test_reg, final_gbr_predictions)
...
# Fit Random Forest and Gradient Boosting classifiers on the training data from question 5 (classification task)
...
# Compute permutation importance on the test set for both classifiers
...
# Select the top-1 feature index from RF based on permutation importance
